...
int32_t QQBot::send_private_message(const std::string & user_id, const std::string & message, bool auto_escape) {
    ...
    if (res && res.value().status == 200) {
        ...
        if (doc.HasMember("status") && std::string(doc["status"].GetString()) == "ok" && doc.HasMember("data")) {
            ...
            WARN("调用API <send_private_msg()> 后，返回的数据无法解析：" + res->body);
            return -1;
        }
    } else if (res && res.value().status != 200) {
        WARN("调用API <send_private_msg()> 后，状态码异常[" + std::to_string(res.value().status) + "]收到的数据：" + res->body);
        return -2;
    } else {
        WARN("调用API <send_private_msg()> 超时，请检查连接！");
        return -3;
    }
}
...
int32_t QQBot::send_group_message(const std::string & group_id, const std::string & message, bool auto_escape) {
    ...
    if (res && res.value().status == 200) {
        ...
        WARN("调用API <send_group_msg()> 后，状态码异常[" + std::to_string(res.value().status) + "]收到的数据："+ res->body);
        return -2;
    } else {
...
int QQBot::delete_msg(const int32_t &message_id) {
    ...
    if (res && res.value().status == 200) {
        ...
        if (doc.HasMember("status") && std::string(doc["status"].GetString()) == "ok") {
            ...
            WARN("调用API <delete_msg()> 后，返回的数据无法解析：" + res->body);
            return -1;
        }
    } else if (res && res.value().status != 200) {
        WARN("调用API <delete_msg()> 后，状态码异常[" + std::to_string(res.value().status) + "]收到的数据：" + res->body);
        return -2;
    } else {
        WARN("调用API <delete_msg()> 超时，请检查连接！");
        return -3;
    }
}
...
int QQBot::send_like(const std::string & user_id, int times) {
    ...
    if (res && res.value().status == 200) {
        ...
        if (doc.HasMember("status") && std::string(doc["status"].GetString()) == "ok") {
            ...
            WARN("调用API <send_like()> 后，点赞失败：" + res->body);
            return 0;
        } else {
            WARN("调用API <send_like()> 后，返回的数据无法解析：" + res->body);
            return -1;
        }
    } else if (res && res.value().status != 200) {
        WARN("调用API <send_like()> 后，状态码异常[" + std::to_string(res.value().status) + "]收到的数据：" + res->body);
        return -2;
    } else {
        WARN("调用API <send_like()> 超时，请检查连接！");
        return -3;
    }
}
...
int QQBot::set_group_kick(const std::string & group_id, const std::string & user_id, bool reject_add_request) {
    ...
    if (res && res.value().status == 200) {
        ...
        if (doc.HasMember("status") && std::string(doc["status"].GetString()) == "ok") {
            ...
            WARN("调用API <set_group_kick()> 后，踢出失败：" + res->body);
            return 0;
        } else {
            WARN("调用API <set_group_kick()> 后，返回的数据无法解析：" + res->body);
            return -1;
        }
    } else if (res && res.value().status != 200) {
        WARN("调用API <set_group_kick()> 后，状态码异常[" + std::to_string(res.value().status) + "]收到的数据：" + res->body);
        return -2;
    } else {
        WARN("调用API <set_group_kick()> 超时，请检查连接！");
        return -3;
    }
}
...
int QQBot::set_group_ban(const std::string & group_id, const std::string & user_id, long long duration) {
    ...
    if (res && res.value().status == 200) {
        ...
        if (doc.HasMember("status") && std::string(doc["status"].GetString()) == "ok") {
            ...
            WARN("调用API <set_group_ban()> 后，禁言失败：" + res->body);
            return 0;
        } else {
            WARN("调用API <set_group_ban()> 后，返回的数据无法解析：" + res->body);
            return -1;
        }
    } else if (res && res.value().status != 200) {
        WARN("调用API <set_group_ban()> 后，状态码异常[" + std::to_string(res.value().status) + "]收到的数据：" + res->body);
        return -2;
    } else {
        WARN("调用API <set_group_ban()> 超时，请检查连接！");
        return -3;
    }
}
...
int QQBot::set_group_whole_ban(const std::string & group_id, bool enable) {
    ...
    if (res && res.value().status == 200) {
        ...
        if (doc.HasMember("status") && std::string(doc["status"].GetString()) == "ok") {
            ...
            WARN("调用API <set_group_whole_ban()> 后，全群禁言失败：" + res->body);
            return 0;
        } else {
            WARN("调用API <set_group_whole_ban()> 后，返回的数据无法解析：" + res->body);
            return -1;
        }
    } else if (res && res.value().status != 200) {
        WARN("调用API <set_group_whole_ban()> 后，状态码异常[" + std::to_string(res.value().status) + "]收到的数据：" + res->body);
        return -2;
    } else {
        WARN("调用API <set_group_whole_ban()> 超时，请检查连接！");
        return -3;
    }
}
...
int QQBot::set_group_admin(const std::string & group_id, const std::string & user_id, bool enable) {
    ...
    if (res && res.value().status == 200) {
        ...
        if (doc.HasMember("status") && std::string(doc["status"].GetString()) == "ok") {
            ...
            WARN("调用API <set_group_admin()> 后，设置管理员失败：" + res->body);
            return 0;
        } else {
            WARN("调用API <set_group_admin()> 后，返回的数据无法解析：" + res->body);
            return -1;
        }
    } else if (res && res.value().status != 200) {
        WARN("调用API <set_group_admin()> 后，状态码异常[" + std::to_string(res.value().status) + "]收到的数据：" + res->body);
        return -2;
    } else {
        WARN("调用API <set_group_admin()> 超时，请检查连接！");
        return -3;
    }
}
...
int QQBot::set_group_card(const std::string & group_id, const std::string & user_id, const std::string & card) {
    ...
    if (res && res.value().status == 200) {
        ...
        if (doc.HasMember("status") && std::string(doc["status"].GetString()) == "ok") {
            ...
            WARN("调用API <set_group_card()> 后，设置名片失败：" + res->body);
            return 0;
        } else {
            WARN("调用API <set_group_card()> 后，返回的数据无法解析：" + res->body);
            return -1;
        }
    } else if (res && res.value().status != 200) {
        WARN("调用API <set_group_card()> 后，状态码异常[" + std::to_string(res.value().status) + "]收到的数据：" + res->body);
        return -2;
    } else {
        WARN("调用API <set_group_card()> 超时，请检查连接！");
        return -3;
    }
}
...
int QQBot::set_group_name(const std::string & group_id, const std::string & group_name) {
    ...
    if (res && res.value().status != 200) {
        ...
        if (doc.HasMember("status") && std::string(doc["status"].GetString()) == "ok") {
            ...
            WARN("调用API <set_group_name()> 后，设置群名称失败：" + res->body);
            return 0;
        } else {
            WARN("调用API <set_group_name()> 后，返回的数据无法解析：" + res->body);
            return -1;
        }
    } else if (res && res.value().status != 200) {
        WARN("调用API <set_group_name()> 后，状态码异常[" + std::to_string(res.value().status) + "]收到的数据：" + res->body.c_str());
        return -2;

    } else {
        WARN("调用API <set_group_name()> 超时，请检查连接！");
        return -2;
    }
}
...
QQBot::login_info QQBot::get_login_info() {
    ...
    if (res && res.value().status == 200) {
        ...
        WARN("调用API <get_login_info()> 后，状态码异常[" + std::to_string(res.value().status) + "]收到的数据：" + res->body.c_str());
        info.status = -2;
        return info;
...
QQBot::bot_status QQBot::get_status() {
    ...
    if (res && res.value().status == 200) {
        ...
        WARN("调用API <get_status()> 后，状态码异常[" + std::to_string(res.value().status) + "]收到的数据：" + res->body.c_str());
        status.status = -2;
        return status;
    } else {
...
std::vector<QQBot::group_list> QQBot::get_group_list() {
    ...
    if (res && res.value().status == 200) {
        ...
        WARN("调用API <get_group_list()> 后，状态码异常[" + std::to_string(res.value().status) + "]收到的数据：" + res->body.c_str());
        return group_list;
    } else {
        group_list.push_back({-3, 0, "", 0, 0});
...
QQBot::group_member_info QQBot::get_group_member_info(const std::string & group_id, const std::string & user_id, bool no_cache) {
    ...
    if (res && res.value().status == 200) {
        ...
        WARN("调用API <get_group_member_info()> 后，状态码异常[" + std::to_string(res.value().status) + "]收到的数据：" + res->body.c_str());
        info.status = -2;
        return info;
    } else {
